export const getCurrentIP = async (): Promise<string | false> => {
	try {
		...
		if (response.status !== 200) {
			console.log(
				`[%c${new Date().toLocaleTimeString()}%c] Got invalid response from the IP service.`,
				'color: red',
				'color: initial',
			);
			return false;
		...
		console.log(
			`[%c${new Date().toLocaleTimeString()}%c] Connection error, failed to grab the latest IP address.`,
			'color: red',
			'color: initial',
		);
		return false;
